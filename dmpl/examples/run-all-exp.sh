...
function do_coll {
    ...
    for N in $NV; do
        for X in $XV; do
            for R in $RV; do
                TD="coll.$1.$X.$X:sync-coll-avoid.$1.dmpl:$N:$R:--DX=$X--DY=$X$2"
...
function do_coll_opt {
    ...
    for N in $NV; do
        for X in $XV; do
            for R in $RV; do
                TD="coll.opt.$1.$X.$X:sync-coll-opt.$1.dmpl:$N:$R:--DX=$X--DY=$X$2"
                if [ $(exists_stats $OUTD "$TD") == "n" ]; then
                    echo "running $TD .."
                    run_brunch "${N}:${X}:${R}" "$TD"
                    update_results $OUTD
...
function do_coll_3d {
    ...
    for N in $NV; do
        for X in $XV; do
            for R in $RV; do
                TD="coll.3d.$1.$X.$X:sync-coll-3d.$1.dmpl:$N:$R:--DX=$X--DY=$X$2"
                if [ $(exists_stats $OUTD "$TD") == "n" ]; then
                    echo "running $TD .."
                    run_brunch "${N}:${X}:${R}" "$TD"
                    update_results $OUTD
...
function do_coll_3d_opt {
    ...
    for N in $NV; do
        for X in $XV; do
            for R in $RV; do
                TD="coll.3d.opt.$1.$X.$X:sync-coll-3d-opt.$1.dmpl:$N:$R:--DX=$X--DY=$X$2"
                if [ $(exists_stats $OUTD "$TD") == "n" ]; then
                    echo "running $TD .."
                    run_brunch "${N}:${X}:${R}" "$TD"
                    update_results $OUTD
...
function do_mutex {
    ...
    for N in $NV; do
        for R in $RV; do
            TD="mutex.$1:sync-mutex.$1.dmpl:$N:$R:$2"
            if [ $(exists_stats $OUTD "$TD") == "n" ]; then
                echo "running $TD .."
                run_brunch "${N}:${R}" "$TD"
                update_results $OUTD
