...
if [ "$#" != "5" ]; then
    echo "Usage : $0 <num-nodes> <num-experiments> <dmpl-file> <dmplc-args> <out-file>"
    echo "        num-experiments = number of experiments"
    echo "        each experiment will do 5 runs in parallel"
    echo 'Example : $0 3 10 coll-avoid.dmpl "--DUSE_DISC_METHOD 1" foo.out (to use disc method)'
    echo 'Example : $0 3 10 coll-avoid.dmpl "--DUSE_DISC_METHOD 0" foo.out (to use conservative method)'
    exit 1
fi
...
DASL_BASE=$(basename $DASL_FILE .dmpl)
CPP_FILE="$DASL_BASE.cpp"
g++ -Wall coll-avoid-experiment.cpp -o coll-avoid-experiment
dmplc $DASLC_ARGS --nodes $NUM_NODES --madara --out $CPP_FILE $DASL_FILE
...
do
    for j in `seq 1 5`; do
        ...
        $DMPL_ROOT/dmpl/examples/coll-avoid/coll-avoid-experiment ./$DASL_BASE $NUM_NODES dom-$j $TMPD 2>&1 >> $TMPD/out &
    done
    wait
done
