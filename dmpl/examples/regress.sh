...
#all the testcases
TESTS=( \
    "sync-coll-avoid.ok.dmpl 2 4 SUCCESS" \
    "sync-coll-avoid.bug1.dmpl 2 4 FAILURE" \
    "sync-coll-avoid.bug2.dmpl 2 4 FAILURE" \
    "sync-mutex.ok.dmpl 2 4 SUCCESS" \
    "sync-mutex.bug1.dmpl 2 4 FAILURE" \
    "sync-mutex.bug2.dmpl 2 4 FAILURE" \
    "sync-coll-opt.ok.dmpl 2 4 SUCCESS" \
    "sync-coll-opt.bug1.dmpl 2 4 FAILURE" \
    "sync-coll-opt.bug2.dmpl 2 4 FAILURE" \
    )

#test parser
function test_parser {
    echo "==== testing dmplc parsing and printing"
    for j in "${TESTS[@]}"; do
        i=$(echo $j | awk '{print $1}')
        dmplc $i --print > $TMPF1
        dmplc $TMPF1 --print > $TMPF2
...
function test_verif_array {
    ...
    for i in "${TESTS[@]}"; do
        CMDS=($i)
        dmplc ${CMDS[0]} --nodes ${CMDS[1]} --rounds ${CMDS[2]} --seq --out $TMPF1.c &> /dev/null
...
function test_verif_no_array {
    ...
    for i in "${TESTS[@]}"; do
        CMDS=($i)
        dmplc ${CMDS[0]} --nodes ${CMDS[1]} --rounds ${CMDS[2]} --seq --seq-no-array --out $TMPF1.c &> /dev/null
        run_cbmc "${CMDS[0]}" $TMPF1.c "${CMDS[3]}"
    done
}
...
function test_verif_sem {
    ...
    for i in "${TESTS[@]}"; do
        CMDS=($i)
        dmplc ${CMDS[0]} --nodes ${CMDS[1]} --rounds ${CMDS[2]} --seq --seq-sem --out $TMPF1.c &> /dev/null
        run_cbmc "${CMDS[0]}" $TMPF1.c "${CMDS[3]}"
    done
}
...
function test_verif_dbl {
    ...
    for i in "${TESTS[@]}"; do
        CMDS=($i)
        dmplc ${CMDS[0]} --nodes ${CMDS[1]} --rounds ${CMDS[2]} --seq --seq-dbl --out $TMPF1.c &> /dev/null
        run_cbmc "${CMDS[0]}" $TMPF1.c "${CMDS[3]}"
    done
}
