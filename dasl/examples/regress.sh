...
function test_verif_no_array {
    ...
    for i in "${TESTS[@]}"; do
        CMDS=($i)
        daslc ${CMDS[0]} --nodes ${CMDS[1]} --rounds ${CMDS[2]} --seq --seq-no-array --out $TMPF1.c &> /dev/null
        run_cbmc "${CMDS[0]}" $TMPF1.c "${CMDS[3]}"
    done
}
...
function test_verif_sem {
    ...
    for i in "${TESTS[@]}"; do
        CMDS=($i)
        daslc ${CMDS[0]} --nodes ${CMDS[1]} --rounds ${CMDS[2]} --seq --seq-sem --out $TMPF1.c &> /dev/null
        run_cbmc "${CMDS[0]}" $TMPF1.c "${CMDS[3]}"
    done
}
...
function test_verif_dbl {
    ...
    for i in "${TESTS[@]}"; do
        CMDS=($i)
        daslc ${CMDS[0]} --nodes ${CMDS[1]} --rounds ${CMDS[2]} --seq --seq-dbl --out $TMPF1.c &> /dev/null
        run_cbmc "${CMDS[0]}" $TMPF1.c "${CMDS[3]}"
    done
}
